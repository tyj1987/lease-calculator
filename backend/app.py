...
def extract_missing_params(data):
    ...
    if isinstance(data, dict):
        ...
        for key, value in data.items():
            ...
            # 特殊格式化处理
            if key in ['pv', 'pmt', 'total_interest', 'total_payment', 'payment', 'principal', 'interest', 'remaining_balance', 'offset_amount', 'remaining_payment', 'guarantee', 'unused_guarantee', 'total_offset']:
...
def sensitivity_analysis():
    ...
    try:
        ...
        if 'base_params' not in data:
            return jsonify({'error': '缺少必要参数: base_params'}), 400
        if 'sensitivity_params' not in data:
            return jsonify({'error': '缺少必要参数: sensitivity_params'}), 400
        
        base_params_raw = data['base_params']
        sensitivity_params = data['sensitivity_params']
        
        # 验证base_params中的必要参数
        required_base_params = ['pv', 'annual_rate', 'periods']
        for param in required_base_params:
            if param not in base_params_raw:
                return jsonify({'error': f'base_params中缺少必要参数: {param}'}), 400
        
        # 标准化base_params，确保与其他模块一致
        base_params = {
            'pv': float(base_params_raw['pv']),
            'annual_rate': float(base_params_raw['annual_rate']),
            'periods': int(base_params_raw['periods']),
            'frequency': int(base_params_raw.get('frequency', 12))  # 默认月付，与其他模块一致
        }
        
        # 验证sensitivity_params
        if not isinstance(sensitivity_params, dict) or not sensitivity_params:
            return jsonify({'error': 'sensitivity_params必须是非空字典'}), 400
        
        result = calculator.sensitivity_analysis(base_params, sensitivity_params)
