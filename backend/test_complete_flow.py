...
def test_complete_calculation_flow():
    ...
    # 调用计算API
    response = requests.post('http://127.0.0.1:5002/api/calculate',
                           json=calc_data,
    ...
    if response.status_code == 200:
        ...
        print("\n📊 测试Excel导出...")
        export_response = requests.post('http://127.0.0.1:5002/api/export/excel',
                                      json=calc_result,
                                      headers={'Content-Type': 'application/json'})
        ...
        print("\n📄 测试JSON导出...")
        json_response = requests.post('http://127.0.0.1:5002/api/export/json',
                                    json=calc_result,
                                    headers={'Content-Type': 'application/json'})
