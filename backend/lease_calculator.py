...
class LeaseCalculator:
    ...
    def equal_annuity_method(self, pv: float, annual_rate: float, periods: int, 
                           frequency: int = 12) -> Dict:
        """
        等额年金法（等额本息法）
    ...
    def equal_principal_method(self, pv: float, annual_rate: float, periods: int,
                             frequency: int = 12) -> Dict:
        """
        等额本金法
    ...
    def flat_rate_method(self, pv: float, flat_rate: float, years: float,
                        frequency: int = 12) -> Dict:
        """
        平息法计算
    ...
    def floating_rate_method(self, pv: float, initial_rate: float, periods: int,
                           rate_reset_schedule: List[Dict], frequency: int = 12) -> Dict:
        """
        浮动利率法
        ...
        # 创建利率变化映射
        rate_changes = {item['period']: Decimal(str(item['new_rate'])) 
    ...
    def _newton_irr(self, cash_flows: List[float], frequency: int = 12, 
    ...
    def apply_guarantee_offset(self, schedule: List[Dict], guarantee: float,
                             mode: str = "尾期冲抵") -> Dict:
        """
        保证金冲抵处理
    ...
    def sensitivity_analysis(self, base_params: Dict, 
        ...
        base_irr = self.calculate_irr(
            [-base_params['pv']] + [base_result['pmt']] * base_params['periods'],
    ...
    def reverse_calculate_rate(self, pv: float, target_pmt: float, periods: int, 
                              frequency: int = 12, method: str = 'equal_annuity',
                              tolerance: float = 0.01, max_iterations: int = 100) -> Dict:
        """
        反向计算年利率
    ...
    def reverse_calculate_pmt(self, pv: float, target_irr: float, periods: int,
                             frequency: int = 12, method: str = 'equal_annuity',
                             tolerance: float = 0.0001, max_iterations: int = 100) -> Dict:
        """
        根据目标IRR反向计算租金
